...
def main():
	...
	bunnies = [Bunny() for i in xrange(5)]
	
	#Initialize a 3 by 3 board filled with 0's
	board = [[0 for col in xrange(25)]for row in xrange(25)]
...
# Handling highscore.txt
	with open("bunnyinfo.txt", 'r') as f:
		currentscore = 0
		for line in f:
			currentscore += 1
	...
	with open("highscore.txt", 'r+') as f:
		...
		if currentscore > top_score:
			print "New top score, ", currentscore
			f.seek(0)
			f.write(str(currentscore)) 
			shutil.copy2('bunnyinfo.txt', 'topscores/' + str(currentscore) + '.txt')
			shutil.copy2('boardtest.txt', 'topscores/' + str(currentscore) + '_board' + '.txt')
		else:
			print currentscore
